...
class Game:
    ...
    def game_system_2_for_second(self):
        self.second_ships = []  # ディーラーの艦隊を保存するリスト
        counter_2 = 0
        print("プレイヤー２の艦隊:")
        while counter_2 < 5:
            ...
            self.second_ships.append(ship_container_2)# ディーラーの艦船のシリアル番号を保存
    ...
    def game_system_4_for_second_select(self):
       while True:
            try:
                select = int(input("使用する軍艦のシリアル番号を半角数字で入力: "))
                for ship_container_2 in self.second_ships:
                    if select == ship_container_2[1]:
                        print(f"選択された艦種: {ship_container_2[0].name_jp}, シリアル番号: {ship_container_2[1]}")
                        return ship_container_2[0]  # 選択された艦船を返す
                print("無効な入力を検出しました。やり直してください。")
            except ValueError:
                print("無効な入力です。数字を入力してください。")
    ...
    def game_system_5_for_battle(self, player_card, dealer_card):
        ...
        print(f"プレイヤー２の {dealer_card.name_jp} がプレイヤー１の {player_card.name_jp} に {dealer_damage} のダメージを与えました。")
        if player_card.hitpoint <= 0:
            print(f"プレイヤー１の {player_card.name_jp} は沈没しました。")
        ...
        print(f"プレイヤー１の {player_card.name_jp} がプレイヤー２の {dealer_card.name_jp} に {player_damage} のダメージを与えました。")
        if dealer_card.hitpoint <= 0:
            print(f"プレイヤー２の {dealer_card.name_jp} は沈没しました。")
            self.second_ships = [ship for ship in self.second_ships if ship[0] != dealer_card]

        while True:
            if not self.player_ships:
                print("プレイヤー１の艦隊は全滅しました。プレイヤー２の勝利！")
                return True  # 戦闘終了
            elif not self.second_ships:
                print("プレイヤー２の艦隊は全滅しました。プレイヤー１の勝利！")
                return True  # 戦闘終了
            else:
                print("戦闘を継続します。")
                # プレイヤーとディーラーの艦船を選択
                ...
                print("プレイヤー２の艦船情報:")
                for ship_container_2 in self.second_ships:
                 print(f"艦種: {ship_container_2[0].name_jp}, シリアル番号: {ship_container_2[1]}, HP: {ship_container_2[0].hitpoint},")        
                print("")
                ...
                for ship_container_2 in self.second_ships:
                 print(f"艦種: {ship_container_2[0].name_jp}, シリアル番号: {ship_container_2[1]}, HP: {ship_container_2[0].hitpoint},")
                dealer_card = self.game_system_4_for_second_select()
                # 戦闘を実行
...
if __name__ == "__main__":
    ...
    print("")
    print("")
    game.game_system_2_for_second()
    print("")
    game.game_system_4_for_second_select()
    print("")
    game.game_system_5_for_battle(game.player_ships[0][0], game.second_ships[0][0])  # 戦闘を開始